...
const Sidebar = () => {
  const activeMenu = true; // Consider using this state if you plan to toggle sidebar visibility
          ...
          <div className="mt-10">
            {links.map((section) => (
              <div key={section.title}>
                <p className='text-gray-400 m-3 mt-4 uppercase'>{section.title}</p>
                {section.links.map((link) => (
                  <NavLink
                    key={link.name}
                    to={`/${link.name}`} // Example: '/ecommerce', '/orders', '/employees', etc.
